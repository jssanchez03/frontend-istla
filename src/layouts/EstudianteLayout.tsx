...
import { LogOut, FileText, SquarePen, User, ChevronDown, AlignJustify } from "lucide-react";

interface Props {
  children: ReactNode;
}
const EstudianteLayout = ({ children }: Props) => {
      ...
      >
        ...
        <nav className="flex-1 px-2 py-6 space-y-1 overflow-y-auto">
          <SidebarLink
            ...
            customActive={location.pathname === "/estudiante" || location.pathname.includes("/estudiante/heteroevaluacion/")}
          />
          <SidebarLink
            to="/estudiante/resumen"
            icon={<FileText className="w-5 h-5" />}
            label="Resumen"
            delay={150}
            isInitialLoad={isInitialLoad}
            currentPath={location.pathname}
            customActive={location.pathname.includes("/estudiante/resumen") || location.pathname.includes("/evaluaciones")}
          />
        </nav>
        ...
        <div className="block md:hidden mt-auto px-4 py-4 border-t border-gray-200">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="font-medium leading-none text-sm">{usuario?.nombre || "Estudiante"}</p>
              <p className="text-gray-500 text-xs">{usuario?.rolTexto || "Estudiante"}</p>
            </div>
          </div>
          <button
      ...
      <div className={`flex-1 flex flex-col transition-all duration-500 ease-out ${sidebarVisible ? 'md:ml-64' : 'ml-0'}`}>
        ...
        >
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <h1 className="text-base font-semibold text-gray-800 truncate block md:hidden">Evaluación Docente</h1>
            <h1 className="hidden md:block text-lg md:text-xl font-semibold text-gray-800 truncate">Sistema de Evaluación Integral Docente</h1>
          </div>
          ...
          <div className="hidden md:flex items-center gap-2 md:gap-4 relative" ref={menuRef}>
            ...
            >
              ...
              <div className="hidden md:block text-sm text-left">
                <p className="font-medium leading-none">{usuario?.nombre || "Estudiante"}</p>
                <p className="text-gray-500 text-xs">{usuario?.rolTexto || "Estudiante"}</p>
              </div>
...
const SidebarLink = ({ to, icon, label, delay, isInitialLoad, currentPath, customActive }: SidebarLinkProps) => {
  const isActive = customActive !== undefined ? customActive : currentPath === to;
  return (
    <Link
      to={to}
