// src/layouts/CoordinadorLayout.tsx
import { type ReactNode, useState, useEffect, useRef } from "react";
import { useAuth } from "../context/AuthContext";
import { Link, useLocation, useNavigate } from "react-router-dom";
import { LogOut, User, ChevronDown, Users, ClipboardList, AlignJustify } from "lucide-react";

interface Props {
    children: ReactNode;
}

const CoordinadorLayout = ({ children }: Props) => {
            ...
            >
                ...
                <nav className="flex-1 px-2 py-6 space-y-1 overflow-y-auto">
                    <SidebarLink
                        ...
                        label="Asignar Coevaluaciones"
                        delay={100}
                        isInitialLoad={isInitialLoad}
                        currentPath={location.pathname}
                    />
                    <SidebarLink
                        to="/coordinador/reporte-coevaluaciones"
                        startsWith
                ...
                <div className="block md:hidden mt-auto px-4 py-4 border-t border-gray-200">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                            <p className="font-medium leading-none text-sm">{usuario?.nombre || "Coordinador"}</p>
                            <p className="text-gray-500 text-xs">{usuario?.rolTexto || "Coordinador"}</p>
                        </div>
                    </div>
                    <button
            ...
            <div className={`flex-1 flex flex-col transition-all duration-500 ease-out ${sidebarVisible ? 'md:ml-64' : 'ml-0'}`}>
                ...
                >
                    <div className="flex items-center gap-2 md:gap-4">
                        ...
                        <h1 className="text-base font-semibold text-gray-800 truncate block md:hidden">Panel Coordinador</h1>
                        <h1 className="hidden md:block text-lg md:text-xl font-semibold text-gray-800 truncate">Panel del Coordinador</h1>
                    </div>
                    ...
                    <div className="hidden md:flex items-center gap-2 md:gap-4 relative" ref={menuRef}>
                        ...
                        >
                            ...
                            <div className="hidden md:block text-sm text-left">
                                <p className="font-medium leading-none">{usuario?.nombre || "Coordinador"}</p>
                                <p className="text-gray-500 text-xs">{usuario?.rolTexto || "Coordinador"}</p>
                            </div>
