...
import AOS from "aos";
import { FileText, CheckCircle, Clock, CheckCheck, Loader2, BookOpen, Calendar, ArrowLeft, PlayCircle, Info, X } from "lucide-react";
...
const CoevaluacionesDocente = () => {
    ...
    const cargarEvaluaciones = async () => {
        try {
            ...
            // Filtrar solo autoevaluaciones (id_formulario = 3)
            const autoevaluaciones = response.data.filter((evaluacion: Evaluacion) => evaluacion.id_formulario === 3);
            setEvaluaciones(autoevaluaciones);
            setError(null);
        } catch (err) {
        ...
        return (
            <div className="max-w-8xl mx-auto p-6">
                <div className="flex justify-center items-center h-64">
                    <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#81bf20]"></div>
                    <span className="ml-3 text-gray-600">Cargando autoevaluaciones...</span>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-8xl mx-auto p-6">
                    ...
                    <div className="flex items-center">
                        <div className="flex-shrink-0 text-[#81bf20]">
                            {progreso < 100 ? (
                                ...
                                <CheckCheck className="h-10 w-10 text-[#81bf20]" />
                            )}
                        </div>
                        <div className="ml-4 w-full">
                            ...
                            <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                                <div
                                    className="bg-[#81bf20] h-2 rounded-full transition-all duration-300"
            ...
            {evaluaciones.length === 0 ? (
                ...
                /* Lista de autoevaluaciones con estilo mejorado */
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex-1">
                                        <div className="flex items-center mb-2">
                                            <div className="w-2 h-2 bg-[#81bf20] rounded-full mr-2"></div>
                                            <span className="text-sm font-medium text-[#81bf20]">
                                                {evalItem.nombre_formulario}
                                            </span>
                                        </div>
                                        <h4 className="text-xl font-bold text-gray-900 mb-2">
                                            {evalItem.nombres_docente || `Coevaluación Docente`}
                                        </h4>
                                    ...
                                    >
                                        ...
                                        ></div>
                                        {evalItem.estado_estudiante === "completada" ? "Completada" : "Por Completar"}
                                    </span>
                                </div>

                                {evalItem.estado_estudiante === "pendiente" && (
                                    <button
                                        className="w-full bg-[#81bf20] text-white px-6 py-3 rounded-lg hover:from-purple-600 hover:to-purple-700 transition-all duration-200 font-semibold flex items-center justify-center"
                                        onClick={() => navigate(`/evaluaciones/co/${evalItem.id_evaluacion}?distributivo=${evalItem.id_distributivo}`)}
                                    >
                        ...
                        <div className="p-6 max-h-[90vh] overflow-y-auto">
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h3 className="text-xl font-semibold text-gray-800">
                                        Información sobre Coevaluaciones
                                    </h3>
                                </div>
                                <button
                                    onClick={() => setMostrarInfo(false)}
                            ...
                            <div className="space-y-6">
                                        ...
                                        <div className="flex items-start gap-3 p-4 bg-blue-50 rounded-lg border border-blue-100">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-blue-800 text-sm">
                                                    Estadísticas de tus coevaluaciones: total, completadas, pendientes y progreso.
                                                </p>
                                            </div>
                                        </div>
                                        <div className="flex items-start gap-3 p-4 bg-green-50 rounded-lg border border-green-100">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-green-800 text-sm">
                                                    Todas tus coevaluaciones pendientes y completadas con detalles de cada docente evaluado.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="p-4 bg-gray-50 rounded-lg">
                                    <h4 className="font-semibold text-gray-800 mb-2">¿Por qué es importante coevaluar?</h4>
                                    <p className="text-sm text-gray-600">
                                        La coevaluación fortalece la comunidad académica y contribuye al desarrollo profesional de tus colegas docentes.
                                    </p>
                                </div>
                            </div>
...
export default CoevaluacionesDocente;